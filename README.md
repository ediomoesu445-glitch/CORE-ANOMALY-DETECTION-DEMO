...
| **2. Classifier** | LightGBM (Gradient Boosting) | Confirms the deviation is a real fault, at 99.50% precision when it raises an alarm |
...
in chunks, which holds peak memory to roughly 600 MB, so it will run on a modest
laptop.
...
fault-free testing file alongside them, for 21 files in `fault_data/`. Load time
drops from ~3 minutes to ~1 second.
